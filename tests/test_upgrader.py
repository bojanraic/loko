...
@pytest.fixture
def sample_config_with_updater(temp_dir):
    """Create a sample config file with loko-updater comments."""
...
@patch('shutil.copy')
def test_upgrade_config_with_updates(mock_copy, mock_fetch, mock_fetch_batch, sample_config_with_updater):
    """Test upgrading config when updates are available."""
    # Mock version fetching to return newer versions
    def fetch_side_effect(updater_info):
        if updater_info['depName'] == 'kindest/node':
            return ("v1.34.0", 0.5)
        elif updater_info['depName'] == 'traefik':
            return ("37.3.0", 1.2)
        return (None, 0.0)
    ...
    upgrade_config(sample_config_with_updater)

    # Verify backup was created
    mock_copy.assert_called_once()

    # Verify updated values in the file
    with open(sample_config_with_updater, 'r') as f:
        content = f.read()
...
@patch('loko.updates.upgrader.fetch_latest_helm_versions_batch')
@patch('loko.updates.upgrader.fetch_latest_version')
def test_upgrade_config_no_updates(mock_fetch, mock_fetch_batch, sample_config_with_updater):
...
@patch('loko.updates.upgrader.fetch_latest_helm_versions_batch')
@patch('loko.updates.upgrader.fetch_latest_version')
def test_upgrade_config_partial_updates(mock_fetch, mock_fetch_batch, sample_config_with_updater):
...
@patch('loko.updates.upgrader.fetch_latest_version')
def test_upgrade_config_fetch_error(mock_fetch, sample_config_with_updater):
...
def test_upgrade_config_no_updater_comments(temp_dir):
    """Test upgrading config with no loko-updater comments."""
    config_path = os.path.join(temp_dir, "loko.yaml")
    config_content = """environment:
  cluster:
    kubernetes:
...
@patch('loko.updates.upgrader.fetch_latest_version')
@patch('shutil.copy')
def test_upgrade_config_preserves_comments(mock_copy, mock_fetch, sample_config_with_updater):
    ...
    # Loko-updater comments should still be present
    assert '# loko-updater: datasource=docker' in updated
    assert '# loko-updater: datasource=helm' in updated


@patch('loko.updates.upgrader.fetch_latest_version')
def test_upgrade_config_backup_filename(mock_fetch, sample_config_with_updater):
    ...
    with patch('shutil.copy') as mock_copy:
        ...
        backup_path = sample_config_with_updater.replace('.yaml', '-prev.yaml')
        mock_copy.assert_called_once_with(sample_config_with_updater, backup_path)
...
"""
    ...
    def fetch_side_effect(updater_info):
        ...
        return versions.get(updater_info['depName'], (None, 0.0))
...
@patch('loko.updates.upgrader.fetch_latest_version')
@patch('shutil.copy')
def test_upgrade_config_timing_metrics(mock_copy, mock_fetch, sample_config_with_updater, capsys):
    """Test that timing metrics are displayed."""
    # Mock with specific timing values
    def fetch_side_effect(updater_info):
        if updater_info['datasource'] == 'docker':
            return ("v1.34.0", 0.5)
        elif updater_info['datasource'] == 'helm':
            return ("37.3.0", 1.5)
        return (None, 0.0)
